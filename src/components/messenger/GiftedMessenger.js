...
class GiftedMessenger extends Component {
  ...
  constructor(props) {
    ...
    if (!this.props.hideTextInput) {
      if (this.props.styles.hasOwnProperty('textInputContainer')) {
        textInputHeight = this.props.styles.textInputContainer.height + 10 || textInputHeight;
      }
    }

    this.listViewMaxHeight = this.props.maxHeight - textInputHeight;
    this.adjustableListHeight = this.props.maxHeight - textInputHeight;
  ...
  onSend() {
    ...
    this.setState({
      height: new Animated.Value(this.listViewMaxHeight - this.state.keyboardHeight)
    });

    this.adjustableListHeight = this.listViewMaxHeight;
  ...
  renderTextInput() {
    if (this.props.hideTextInput === false) {
      return (
        <View style={[this.styles.textInputContainer, {height: this.props.styles.textInputContainer.height + 10}]}>
          {this.props.leftControlBar}
          <TextInput
            style={[this.styles.textInput, {height: this.props.styles.textInputContainer.height}]}
            placeholder={this.props.placeholder}
            placeholderTextColor={this.props.placeholderTextColor}
            onChangeText={this.onChangeText}
            onChange={(event) => {
              if (event.nativeEvent.contentSize.height > this.props.styles.textInputContainer.height) {
                console.log('LISTVIEW HEIGHT: ', this._listHeight);
                this.adjustableListHeight -= 18;

                this.setState({
                  height: new Animated.Value(this.adjustableListHeight - this.state.keyboardHeight),
                });
              } else if(event.nativeEvent.contentSize.height < this.props.styles.textInputContainer.height) {
                console.log('LISTVIEW HEIGHT: ', this._listHeight);
                this.adjustableListHeight += 18;
